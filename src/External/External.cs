...
namespace External
{
	...
	public class Predictions
	{
		public Predictions(string location, int noahStationId, float lat, float lng, int lookaheadhours)
		{
			...
			AllDataLock = new ReaderWriterLockSlim();
		...
		public List<Data> CurrentTides
		{
			get
			{
				return LatestPredictions(PredictionType.Tides, LookaheadHours, lookaheadmultiplier: 3, retrieveAdditional: (start, end) => 
				{
					var json = GetWebJson(string.Format(PredictionUrl, start, end, NoahStationId, "mllw"));
					return new List<string>() { json };
				});
			}
		}

		public List<Data> CurrentExtremes
		{
			get
			{
				return LatestPredictions(PredictionType.Extremes, LookaheadHours, lookaheadmultiplier: 14, retrieveAdditional: (start, end) =>
				{
					var json = GetWebJson(string.Format(PredictionHiloUrl, start, end, NoahStationId, "mllw"));
					return new List<string>() { json };
				});
			}
		}

		public List<Data> CurrentSuns
		{
			get
			{
				return LatestPredictions(PredictionType.Suns, LookaheadHours, lookaheadmultiplier: 7, retrieveAdditional: (start, end) =>
				{
					...
					{
						json.Add( GetWebJson(string.Format(SunriseSunsetUrl, Latitude, Longitude, date)) );
						date = date.AddDays(1);
					}
		...
		public List<Data> CurrentWeather
        {
			get
            {
				return LatestPredictions(PredictionType.Weather, lookaheadhours: 0, lookaheadmultiplier: 0, retrieveAdditional: (start, end) =>
				{
					// get the grid where to query for weather
					var grid = ParseJson(GetWebJson(string.Format(WeatherInfoUrl, Latitude, Longitude)), PredictionType.WeatherInfo);
		...
		private ReaderWriterLockSlim AllDataLock;
		...
		private List<Data> LatestPredictions(PredictionType type, int lookaheadhours, int lookaheadmultiplier, Func<DateTime, DateTime, List<string>> retrieveAdditional)
		{
			try
			{
				AllDataLock.EnterUpgradeableReadLock();
				...
				if (!AllData.TryGetValue(type, out details))
				{
					...
					try
					{
						AllDataLock.EnterWriteLock();
						AllData.Add(type, details);
					}
					finally
					{
						AllDataLock.ExitWriteLock();
					}
				}
				...
				do
				{
					...
					if (latest == default(DateTime) || datetimewindow > latest)
					{
						...
						foreach (var json in retrieveAdditional(past, future))
						{
			...
			{
				AllDataLock.ExitUpgradeableReadLock();
			}
		}

		private string GetWebJson(string url)
		{
			...
			while(--tries > 0)
			{
				try
				{
					...
					request.UserAgent = $"tids {Latitude + Longitude}";
